...
export default function FacilitiesPage() {
  return (
    <>
      <PageHeader eyebrow="xGI Initiative" title="Facilities and Testbeds" intro={OVERVIEW} />

      <section className="container-site py-16 md:py-20">
        <div className="grid gap-6 sm:grid-cols-2">
          {facilities.map((fac) => (
            <article key={fac.name} className="card flex flex-col overflow-hidden">
              {fac.image ? (
                ...
                <ImagePlaceholder label={`${fac.name} — facility photo`} className="aspect-[16/9] w-full" />
              )}
